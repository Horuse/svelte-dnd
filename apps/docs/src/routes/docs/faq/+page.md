...
Use a single `DndController` shared across all containers. Read `targetContainerId` inside `onDrop` to determine where the item landed:
...
<script>
    ...
    controller.onDrop((sourceId, sourceData, targetContainerId, position) => {
        // find source container, move item to target container at position
    });
</script>
...
Note: in normal usage you don't place `DndPreview` manually — `DndDraggable` and `DndDroppable` handle this automatically. Only check this if you're using `DndPreview` in a custom layout.

...
- Add `onBeforeDrop` lifecycle event, fired when a valid drop is accepted before the ghost starts flying to the target. Use it for optimistic metadata (counters, badges) that must not move the item between containers.
- Add `onDropCommit` lifecycle event, fired when the ghost lands on the target, immediately before `onDrop`. Use it to apply state at the exact moment the drop animation completes.
...
[v1.1.0]: https://github.com/Horuse/svelte-dnd/compare/v1.0.1...v1.1.0
